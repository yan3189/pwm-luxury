...
// Admin: Manajemen Upselling, Bonus, Voucher
// ============================================================
...
import { 
  ...
  Eye, EyeOff, RefreshCw, Search, X
} from 'lucide-react';
import { getUpsells, getBonuses, createUpsell, updateUpsell, deleteUpsell, toggleUpsell } from '../services/upsellService';
...
export default function AdminPromotions() {
  ...
  const loadData = async (storeId) => {
    setLoading(true);
    try {
      const upsellData = await getUpsells(storeId);
  ...
  const handleSave = async () => {
    ...
    try {
      if (activeTab === 'upsells') {
        if (editingItem) {
          await updateUpsell(editingItem.id, form);
        } else {
          await createUpsell(form);
        }
    ...
    if (activeTab === 'upsells') {
      return (
        <div className="space-y-3">
          <div><label className="block text-sm text-gray-400">Judul</label><input type="text" className="w-full p-2 rounded bg-black/50 border border-white/20" value={form.title || ''} onChange={e => setForm({...form, title: e.target.value})} /></div>
          ...
          <div><label className="block text-sm text-gray-400">Harga</label><input type="number" className="w-full p-2 rounded bg-black/50 border border-white/20" value={form.price || 0} onChange={e => setForm({...form, price: parseInt(e.target.value) || 0})} /></div>
          ...
          <div><label className="block text-sm text-gray-400">Produk ID (opsional)</label><input type="text" className="w-full p-2 rounded bg-black/50 border border-white/20" value={form.product_id || ''} onChange={e => setForm({...form, product_id: e.target.value})} /></div>
        </div>
      );
    } else if (activeTab === 'bonuses') {
      return (
  ...
  const getFilteredData = () => {
    ...
    if (activeTab === 'upsells') {
      return upsells.filter(item => item.title?.toLowerCase().includes(query));
    } else if (activeTab === 'bonuses') {
    ...
    <div className="bg-black min-h-screen text-white p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
          ...
          >
            <Plus size={16} /> Tambah {activeTab === 'upsells' ? 'Upselling' : activeTab === 'bonuses' ? 'Bonus' : 'Voucher'}
          </button>
        </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-800/50 border-b border-white/10">
              <tr>
                ...
                {activeTab === 'upsells' && <th className="p-3 text-left">Harga</th>}
              ...
              {data.length === 0 ? (
                ...
                data.map(item => (
                  <tr key={item.id} className="border-b border-white/5 hover:bg-white/5">
                    ...
                    {activeTab === 'upsells' && (
                      <td className="p-3 text-yellow-500">Rp {item.price?.toLocaleString()}</td>
                    )}
                    {activeTab === 'vouchers' && (
                      <>
                    ...
                    <td className="p-3 text-center">
                      <button
                        onClick={() => handleToggle(item.id, activeTab === 'upsells' ? 'upsell' : activeTab === 'bonuses' ? 'bonus' : 'voucher')}
                    ...
                    <td className="p-3 text-right flex gap-2 justify-end">
                      ...
                      <button onClick={() => handleDelete(item.id, activeTab === 'upsells' ? 'upsell' : activeTab === 'bonuses' ? 'bonus' : 'voucher')} className="text-red-400 hover:text-red-300">
                        <Trash2 size={16} />
                      </button>
                    </td>
                  </tr>
          ...
          <div className="bg-gray-900 rounded-2xl p-6 w-full max-w-lg border border-white/10 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-display">
                {editingItem ? 'Edit' : 'Tambah'} {activeTab === 'upsells' ? 'Upselling' : activeTab === 'bonuses' ? 'Bonus' : 'Voucher'}
              </h2>
